...
PLOT_OUTPUT_DIR = r"C:\Users\KDT-24\final-project\Influenza_A_H3N2\#Last_Korea+China+Japan\#32 png"
...
def save_feature_visualizations(final_model: Pipeline, train_df: pd.DataFrame):
    os.makedirs(PLOT_OUTPUT_DIR, exist_ok=True)
    x_raw = train_df[FEATURES].fillna(0)
    x_scaled = final_model.named_steps["scaler"].transform(x_raw)
    x_scaled_df = pd.DataFrame(x_scaled, columns=FEATURES)
    coef = final_model.named_steps["clf"].coef_[0]
    shap_like = x_scaled_df.mul(coef, axis=1)
    coef_df = pd.DataFrame({"feature": FEATURES, "coef": coef, "abs_coef": np.abs(coef)})
    coef_df = coef_df.sort_values("abs_coef", ascending=True)
    plt.figure(figsize=(9, 5))
    colors = ["#d62728" if v > 0 else "#1f77b4" for v in coef_df["coef"]]
    ...
    plt.tight_layout()
    coef_path = os.path.join(PLOT_OUTPUT_DIR, "coef_bar.png")
    plt.savefig(coef_path, dpi=220)
    plt.close()
    mean_abs = shap_like.abs().mean().sort_values(ascending=True)
    plt.figure(figsize=(9, 5))
    plt.barh(mean_abs.index, mean_abs.values, color="#2ca02c")
    plt.title("Feature Importance by Mean |Contribution| (SHAP-like)")
    plt.xlabel("Mean absolute contribution (log-odds)")
    plt.tight_layout()
    shap_bar_path = os.path.join(PLOT_OUTPUT_DIR, "shap_like_importance_bar.png")
    plt.savefig(shap_bar_path, dpi=220)
    plt.close()
    ordered = mean_abs.sort_values(ascending=False).index.tolist()
    plt.figure(figsize=(10, 6))
    rng = np.random.default_rng(RANDOM_STATE)
    for i, f in enumerate(ordered):
        y = np.full(len(shap_like), i) + rng.uniform(-0.22, 0.22, size=len(shap_like))
        plt.scatter(shap_like[f], y, s=10, alpha=0.45)
    plt.yticks(range(len(ordered)), ordered)
    plt.axvline(0, color="black", linewidth=1)
    plt.title("SHAP-like Beeswarm (per-sample log-odds contribution)")
    plt.xlabel("Contribution")
    plt.tight_layout()
    beeswarm_path = os.path.join(PLOT_OUTPUT_DIR, "shap_like_beeswarm.png")
    plt.savefig(beeswarm_path, dpi=220)
    plt.close()
    corr = train_df[FEATURES].fillna(0).corr(numeric_only=True)
    ...
    plt.tight_layout()
    corr_path = os.path.join(PLOT_OUTPUT_DIR, "feature_correlation_heatmap.png")
    plt.savefig(corr_path, dpi=220)
    plt.close()
    top_dep = mean_abs.sort_values(ascending=False).index[:TOP_DEPENDENCE_FEATURES].tolist()
    for feat in top_dep:
        plt.figure(figsize=(7.2, 5.2))
        plt.scatter(x_scaled_df[feat], shap_like[feat], s=14, alpha=0.6)
        plt.axhline(0, color="black", linewidth=1)
        plt.title(f"Dependence: {feat}")
        plt.xlabel(f"{feat} (scaled)")
        plt.ylabel("Contribution")
        plt.tight_layout()
        dep_path = os.path.join(PLOT_OUTPUT_DIR, f"dependence_{feat}.png")
        plt.savefig(dep_path, dpi=220)
        plt.close()

    combined_dep_path = os.path.join(PLOT_OUTPUT_DIR, "dependence_top3_combined.png")
    fig, axes = plt.subplots(1, len(top_dep), figsize=(6.0 * len(top_dep), 5.0), sharey=True)
    if len(top_dep) == 1:
        axes = [axes]
    for ax, feat in zip(axes, top_dep):
        ax.scatter(x_scaled_df[feat], shap_like[feat], s=14, alpha=0.6)
        ax.axhline(0, color="black", linewidth=1)
        ax.set_title(feat)
        ax.set_xlabel(f"{feat} (scaled)")
    axes[0].set_ylabel("Contribution")
    fig.suptitle("Dependence (Top Features) - Combined", y=1.02)
    fig.tight_layout()
    fig.savefig(combined_dep_path, dpi=220, bbox_inches="tight")
    plt.close(fig)

    long_df = x_scaled_df.melt(var_name="feature", value_name="value")
    value_dist_path = os.path.join(PLOT_OUTPUT_DIR, "feature_value_distribution_combined.png")
    plt.figure(figsize=(10.5, 5.8))
    order = mean_abs.sort_values(ascending=False).index.tolist()
    pos = np.arange(len(order))
    for i, feat in enumerate(order):
        vals = long_df.loc[long_df["feature"] == feat, "value"].to_numpy()
        y = np.full(len(vals), i) + np.random.default_rng(RANDOM_STATE + i).uniform(-0.18, 0.18, size=len(vals))
        plt.scatter(vals, y, s=9, alpha=0.35)
    plt.yticks(pos, order)
    plt.axvline(0, color="black", linewidth=1)
    plt.xlabel("Feature value (scaled)")
    plt.title("Feature Value Distribution (All Features, Combined)")
    plt.tight_layout()
    plt.savefig(value_dist_path, dpi=220)
    plt.close()
    summary = pd.DataFrame({
        "feature": mean_abs.sort_values(ascending=False).index,
        "mean_abs_contribution": mean_abs.sort_values(ascending=False).values,
        "coef": pd.Series(coef, index=FEATURES)[mean_abs.sort_values(ascending=False).index].values,
    })
    summary_path = os.path.join(PLOT_OUTPUT_DIR, "feature_importance_summary.csv")
    summary.to_csv(summary_path, index=False, encoding="utf-8-sig")
    shap_real_beeswarm_path = None
    shap_real_bar_path = None
    if shap is not None:
        try:
            bg_n = min(80, len(x_scaled_df))
            bg = x_scaled_df.sample(n=bg_n, random_state=RANDOM_STATE) if bg_n > 0 else x_scaled_df
            explainer = shap.LinearExplainer(final_model.named_steps["clf"], bg)
            shap_values = explainer(x_scaled_df)

            plt.figure(figsize=(10, 6))
            shap.summary_plot(shap_values.values, x_scaled_df, feature_names=FEATURES, show=False)
            plt.tight_layout()
            shap_real_beeswarm_path = os.path.join(PLOT_OUTPUT_DIR, "shap_real_summary_beeswarm.png")
            plt.savefig(shap_real_beeswarm_path, dpi=220)
            plt.close()
            ...
            plt.tight_layout()
            shap_real_bar_path = os.path.join(PLOT_OUTPUT_DIR, "shap_real_summary_bar.png")
            plt.savefig(shap_real_bar_path, dpi=220)
            plt.close()
        except Exception as exc:
            print(f"[Warn] SHAP summary plot skipped: {exc}")

    return {
        "coef_bar": coef_path,
        "shap_bar": shap_bar_path,
        "shap_beeswarm": beeswarm_path,
        "shap_real_beeswarm": shap_real_beeswarm_path,
        "shap_real_bar": shap_real_bar_path,
        "corr_heatmap": corr_path,
        "summary_csv": summary_path,
        "top_dependence": top_dep,
        "combined_dependence": combined_dep_path,
        "combined_value_distribution": value_dist_path,
    }
# ================================================================
# 4. Main
# ================================================================
...
if __name__ == "__main__":
    ...
    plot_paths = save_feature_visualizations(final_model, train_df)
    print("\n[Feature Visualization Saved]")
    print(f"  coef_bar:      {plot_paths['coef_bar']}")
    print(f"  shap_bar:      {plot_paths['shap_bar']}")
    print(f"  shap_beeswarm: {plot_paths['shap_beeswarm']}")
    if plot_paths["shap_real_beeswarm"] is not None:
        print(f"  shap_real_beeswarm: {plot_paths['shap_real_beeswarm']}")
    if plot_paths["shap_real_bar"] is not None:
        print(f"  shap_real_bar:      {plot_paths['shap_real_bar']}")
    print(f"  corr_heatmap:  {plot_paths['corr_heatmap']}")
    print(f"  summary_csv:   {plot_paths['summary_csv']}")
    print(f"  dependence(top {TOP_DEPENDENCE_FEATURES}): {plot_paths['top_dependence']}")
    print(f"  dependence_combined: {plot_paths['combined_dependence']}")
    print(f"  value_dist_combined: {plot_paths['combined_value_distribution']}")
