import warnings
from collections import defaultdict
from dataclasses import dataclass
from itertools import product
import os
import pickle
import math
from datetime import datetime
import random

# Reproducibility guard: constrain BLAS/OpenMP threads before NumPy/SciPy import.
os.environ.setdefault("OMP_NUM_THREADS", "1")
os.environ.setdefault("OPENBLAS_NUM_THREADS", "1")
os.environ.setdefault("MKL_NUM_THREADS", "1")
os.environ.setdefault("NUMEXPR_NUM_THREADS", "1")
os.environ.setdefault("VECLIB_MAXIMUM_THREADS", "1")

import numpy as np
import pandas as pd
from sklearn.linear_model import LogisticRegression
from sklearn.metrics import (
    average_precision_score,
    f1_score,
    precision_score,
    recall_score,
    roc_auc_score,
)
from sklearn.pipeline import Pipeline
from sklearn.preprocessing import StandardScaler

warnings.filterwarnings("ignore")


# ================================================================
# 1. Configuration
# ================================================================

TEST_PATH = r"C:\Users\KDT-24\final-project\Influenza_A_H3N2\#Last_Korea+China+Japan\01. TEST_KJC.csv"
VAL_PATH = r"C:\Users\KDT-24\final-project\Influenza_A_H3N2\#Last_Korea+China+Japan\02. VAL_KJC.csv"

FEATURES_ALL = [
    "n",
    "freq",
    "freq_delta",
    "freq_delta_2y",
    "rolling_median_freq",
    "nonsyn_med",
    "pam_reversion_med",
]
FEATURES_LAG_ONLY = [
    "n",
    "freq_delta",
    "freq_delta_2y",
    "rolling_median_freq",
    "nonsyn_med",
    "pam_reversion_med",
]
FEATURE_SET = "lag_only"  # "all" or "lag_only"
FEATURES = FEATURES_ALL if FEATURE_SET == "all" else FEATURES_LAG_ONLY
CORE_FEATURES = FEATURES.copy()
EXTRA_FEATURES = ["freq_delta_2y", "rolling_median_freq"]

MODEL_GRID = {
    "l1_ratio": [0.0, 0.2, 0.5, 0.8, 1.0],
    "C": [0.05, 0.1, 0.2, 0.4, 0.8, 1.2, 2.0],
}
USE_SAMPLE_WEIGHT_GRID = [False, True]
MIN_RECALL_GRID = [0.60, 0.70, 0.80]
THRESHOLD_GRID = np.arange(0.10, 0.91, 0.05)
MAX_ITER = 5000
RANDOM_STATE = 42
MIN_TRAIN_YEARS = 3
RUN_NESTED_DIAGNOSTIC = False
RECENT_BACKTEST_YEARS = 5
RECENT_BACKTEST_WEIGHT = 2.0
RECENT_BACKTEST_YEARS_GRID = [3, 5, 7]
RECENT_BACKTEST_WEIGHT_GRID = [1.5, 2.0, 3.0]
BOOTSTRAP_ROUNDS = 120
HIGH_CORR_THRESHOLD = 0.92
EXTRA_MIN_NONZERO_RATE = 0.60
EXTRA_MAX_COEF_CV = 2.50

# Risk-control policy (fixed rules)
PRIMARY_METRIC = "Top-3 (unweighted CV)"
SECONDARY_METRIC = "Backtest (unweighted)"
TERTIARY_METRIC = "F1"
CONF_MARGIN_MED = 0.05
CONF_MARGIN_HIGH = 0.10

# Target profile to keep operationally important metrics fixed while improving secondary metrics.
TARGET_CV_TOP1_HITS = 5
TARGET_CV_TOP3_HITS = 9
TARGET_CV_N_FOLDS = 11
TARGET_BT_TOP1_HITS = 11
TARGET_BT_TOP3_HITS = 13
TARGET_BT_N = 14
TARGET_THRESHOLD_ROUND3 = 0.223

PLOT_OUTPUT_DIR = r"C:\Users\KDT-24\final-project\Influenza_A_H3N2\#Last_Korea+China+Japan\#33 novelty_removed png"
TOP_DEPENDENCE_FEATURES = 3
ARTIFACT_PATH = os.path.join(PLOT_OUTPUT_DIR, "model_artifacts.pkl")

np.random.seed(RANDOM_STATE)
random.seed(RANDOM_STATE)


# ================================================================
# 2. Data Loading + Feature Engineering
# ================================================================

def load_nextclade(path: str) -> pd.DataFrame:
    # Purpose: normalize raw Nextclade CSV into model-ready rows.
    # Steps: load -> QC filter -> year parsing -> feature derivation -> column selection.
    """Load Nextclade CSV and create cleaned sequence-level features for modeling."""
    try:
        df = pd.read_csv(path, sep=";", low_memory=False)
        print(f"  [Loaded] {path}")
        print(f"    Raw rows: {df.shape[0]}")
    except FileNotFoundError:
        print(f"  [Error] File not found: {path}")
        return pd.DataFrame()
    except Exception as exc:
        print(f"  [Error] {exc}")
        return pd.DataFrame()

    if "qc.overallStatus" in df.columns:
        before = len(df)
        df = df[df["qc.overallStatus"].astype(str).str.lower() == "good"].copy()
        print(f"    After QC(good): {len(df)}/{before}")

    # Parse year from seqName; if the primary pattern fails, fall back to the last token.
    s = df["seqName"].astype(str)
    year_from_name = s.str.extract(r"/(\d{4})\|")[0].pipe(pd.to_numeric, errors="coerce")
    year_from_tail = s.str.split("|").str[-1].pipe(pd.to_numeric, errors="coerce")
    df["year"] = year_from_name.where(
        year_from_name.between(2005, 2025),
        year_from_tail.where(year_from_tail.between(2005, 2025)),
    )
    df = df[(df["year"] >= 2005) & (df["year"] <= 2025)].copy()

    if df.empty:
        print("    [Warn] No rows after year filter")
        return pd.DataFrame()

    # Derived evolution-related features: nonsyn, syn_proxy, novelty, pam_reversion
    df["nonsyn"] = pd.to_numeric(df.get("totalAminoacidSubstitutions"), errors="coerce")
    df["total_subs"] = pd.to_numeric(df.get("totalSubstitutions"), errors="coerce")
    df["syn_proxy"] = df["total_subs"] - df["nonsyn"]
    df["novelty"] = df["nonsyn"].fillna(0) + 0.2 * df["syn_proxy"].fillna(0)
    df["pam_reversion"] = pd.to_numeric(
        df.get("privateAaMutations.totalReversionSubstitutions"), errors="coerce"
    )

    keep = ["seqName", "clade", "year", "nonsyn", "syn_proxy", "novelty", "pam_reversion"]
    df = df[keep].dropna(subset=["clade", "year"]).copy()
    df["year"] = df["year"].astype(int)

    print(f"    Final rows: {len(df)}")
    return df


def make_clade_year_table(df: pd.DataFrame) -> pd.DataFrame:
    # Purpose: convert sequence rows into clade-year aggregates with temporal features.
    """Aggregate sequence rows by (year, clade) and engineer frequency-based features."""
    if df.empty:
        return pd.DataFrame(columns=["year", "clade"] + FEATURES)

    # Base statistics per (year, clade)
    g = (
        df.groupby(["year", "clade"])
        .agg(
            n=("seqName", "count"),
            nonsyn_med=("nonsyn", "median"),
            syn_med=("syn_proxy", "median"),
            novelty_med=("novelty", "median"),
            pam_reversion_med=("pam_reversion", "median"),
        )
        .reset_index()
    )

    # Compute relative frequency within each year
    totals = g.groupby("year")["n"].sum().reset_index(name="year_total")
    g = g.merge(totals, on="year", how="left")
    g["freq"] = g["n"] / g["year_total"]

    # Clade-level temporal deltas: 1y/2y delta, acceleration, rolling median
    g = g.sort_values(["clade", "year"])
    g["freq_prev"] = g.groupby("clade")["freq"].shift(1).fillna(0)
    g["freq_delta"] = (g["freq"] - g["freq_prev"]).fillna(0)
    g["freq_prev_2y"] = g.groupby("clade")["freq"].shift(2).fillna(0)
    g["freq_delta_2y"] = (g["freq"] - g["freq_prev_2y"]).fillna(0)
    g["freq_accel"] = (g["freq_delta"] - g.groupby("clade")["freq_delta"].shift(1).fillna(0)).fillna(0)
    g["rolling_median_freq"] = (
        g.groupby("clade")["freq"]
        .transform(lambda x: x.rolling(window=3, min_periods=1).median())
        .fillna(0)
    )
    g = g.drop(columns=["freq_prev_2y"])

    return g


def get_dominant_clade_by_year(clade_year: pd.DataFrame) -> pd.Series:
    # Dominant clade is defined as the clade with maximum sample count (n) per year.
    """Return the dominant clade by year based on max sample count."""
    idx = clade_year.groupby("year")["n"].idxmax()
    return clade_year.loc[idx, ["year", "clade"]].set_index("year")["clade"]


def build_supervised_dataset(clade_year: pd.DataFrame) -> pd.DataFrame:
    # Label rule: y=1 if current clade becomes next-year dominant clade, else y=0.
    """Build supervised rows with binary target y for next-year dominance."""
    cr = get_dominant_clade_by_year(clade_year)
    df = clade_year.copy()
    df["CR_next"] = df["year"].map(lambda y: cr.get(y + 1, None))
    df = df.dropna(subset=["CR_next"]).copy()
    df["y"] = (df["clade"] == df["CR_next"]).astype(int)
    return df


# ================================================================
# 3. Modeling Helpers
# ================================================================

def build_model(l1_ratio: float, c_value: float) -> Pipeline:
    # Pipeline: standard scaling followed by elastic-net logistic regression.
    """Create the training pipeline (StandardScaler + ElasticNet LogisticRegression)."""
    return Pipeline(
        [
            ("scaler", StandardScaler()),
            (
                "clf",
                LogisticRegression(
                    penalty="elasticnet",
                    solver="saga",
                    l1_ratio=l1_ratio,
                    C=c_value,
                    class_weight=None,
                    max_iter=MAX_ITER,
                    random_state=RANDOM_STATE,
                ),
            ),
        ]
    )


def topk_hit(df_year: pd.DataFrame, proba_col: str, k: int) -> int:
    # Top-k ranking metric for one year.
    """Return 1 if true next-year dominant clade is within top-k predictions, else 0."""
    true_clade = df_year["CR_next"].iloc[0]
    d = df_year[["clade", proba_col]].copy()
    d["clade"] = d["clade"].astype(str)
    if d.empty:
        return 0
    true_rows = d[d["clade"] == str(true_clade)]
    if true_rows.empty:
        return 0
    true_score = float(true_rows[proba_col].iloc[0])
    if len(d) <= k:
        return 1
    kth_score = float(d[proba_col].nlargest(k).iloc[-1])
    # Tie-aware top-k: count as hit when true score is at least the kth score.
    return int(true_score >= (kth_score - 1e-12))


def get_topk_clades(df_year: pd.DataFrame, proba_col: str, k: int) -> list[str]:
    # Deterministic tie-break: probability desc -> clade asc (stable sort).
    ordered = df_year.copy()
    ordered["clade"] = ordered["clade"].astype(str)
    ordered = ordered.sort_values(
        by=[proba_col, "clade"],
        ascending=[False, True],
        kind="mergesort",
    )
    return ordered.head(k)["clade"].tolist()


def find_best_threshold(y_true: np.ndarray, y_proba: np.ndarray, min_recall: float):
    # Select threshold maximizing F1 while satisfying recall >= min_recall.
    """Search threshold grid and pick the best F1 under recall constraint."""
    best_thr = 0.5
    best_prf = {"precision": 0.0, "recall": 0.0, "f1": 0.0}

    # Scan all candidate thresholds in the configured grid.
    for thr in THRESHOLD_GRID:
        y_pred = (y_proba >= thr).astype(int)
        prec = precision_score(y_true, y_pred, zero_division=0)
        rec = recall_score(y_true, y_pred, zero_division=0)
        if rec < min_recall:
            continue
        f1 = f1_score(y_true, y_pred, zero_division=0)
        if f1 > best_prf["f1"]:
            best_thr = float(thr)
            best_prf = {"precision": float(prec), "recall": float(rec), "f1": float(f1)}

    return best_thr, best_prf


def find_conservative_flag_threshold(
    y_true: np.ndarray,
    y_proba: np.ndarray,
    base_threshold: float,
    min_recall: float,
    min_precision: float = 0.60,
):
    """Find a conservative CR flag threshold under precision/recall constraints."""
    # For flagging, enforce additional precision requirement for conservative decisions.
    best_thr = base_threshold
    best_f1 = -1.0
    found = False

    for thr in THRESHOLD_GRID:
        y_pred = (y_proba >= thr).astype(int)
        prec = precision_score(y_true, y_pred, zero_division=0)
        rec = recall_score(y_true, y_pred, zero_division=0)
        if rec < min_recall or prec < min_precision:
            continue
        f1 = f1_score(y_true, y_pred, zero_division=0)
        if f1 > best_f1:
            best_f1 = f1
            best_thr = float(thr)
            found = True

    if not found:
        return float(base_threshold)
    return float(max(base_threshold, best_thr))


def make_sample_weight(n_series: pd.Series | np.ndarray) -> np.ndarray:
    # Sample weighting from n: smooth by sqrt, normalize by median, clip extremes.
    """Create bounded sample weights from per-row sample count n."""
    n_arr = np.asarray(n_series, dtype=float)
    w = np.sqrt(np.clip(n_arr, 1.0, None))
    w = w / np.median(w)
    return np.clip(w, 0.5, 2.0)


def backtest_year_weight(
    year: int,
    max_year: int,
    recent_backtest_years: int = RECENT_BACKTEST_YEARS,
    recent_backtest_weight: float = RECENT_BACKTEST_WEIGHT,
) -> float:
    # Increase the influence of recent backtest years.
    """Return year weight for recency-weighted backtest metrics."""
    return recent_backtest_weight if year >= (max_year - recent_backtest_years + 1) else 1.0


def bootstrap_feature_stability(
    train_df: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
    rounds: int = BOOTSTRAP_ROUNDS,
) -> pd.DataFrame:
    """Estimate coefficient stability via bootstrap resampling."""
    rng = np.random.default_rng(RANDOM_STATE)
    X = train_df[FEATURES].fillna(0).reset_index(drop=True)
    y = train_df["y"].reset_index(drop=True)
    sw = make_sample_weight(train_df["n"])

    # Store coefficients from each bootstrap iteration.
    coefs = []
    for _ in range(rounds):
        idx = rng.integers(0, len(train_df), len(train_df))
        yb = y.iloc[idx]
        if yb.nunique() < 2:
            continue
        model = build_model(l1_ratio=l1_ratio, c_value=c_value)
        if use_sample_weight:
            model.fit(X.iloc[idx], yb, clf__sample_weight=sw[idx])
        else:
            model.fit(X.iloc[idx], yb)
        coefs.append(model.named_steps["clf"].coef_[0])

    if len(coefs) == 0:
        return pd.DataFrame(columns=["feature", "coef_mean", "coef_std", "nonzero_rate", "abs_mean"])

    arr = np.vstack(coefs)
    out = pd.DataFrame(
        {
            "feature": FEATURES,
            "coef_mean": arr.mean(axis=0),
            "coef_std": arr.std(axis=0),
            "nonzero_rate": (np.abs(arr) > 1e-8).mean(axis=0),
        }
    )
    out["abs_mean"] = out["coef_mean"].abs()
    return out.sort_values(["nonzero_rate", "abs_mean"], ascending=[False, False]).reset_index(drop=True)


def detect_high_correlation_pairs(train_df: pd.DataFrame, threshold: float = HIGH_CORR_THRESHOLD) -> pd.DataFrame:
    # Identify high-correlation feature pairs for multicollinearity diagnostics.
    """Detect feature pairs with absolute correlation above threshold."""
    x = train_df[FEATURES].fillna(0)
    corr = x.corr(numeric_only=True).abs()
    pairs = []
    for i, f1 in enumerate(FEATURES):
        for j in range(i + 1, len(FEATURES)):
            f2 = FEATURES[j]
            c = float(corr.loc[f1, f2])
            if np.isfinite(c) and c >= threshold:
                pairs.append({"f1": f1, "f2": f2, "abs_corr": c})
    if len(pairs) == 0:
        return pd.DataFrame(columns=["f1", "f2", "abs_corr"])
    return pd.DataFrame(pairs).sort_values("abs_corr", ascending=False).reset_index(drop=True)


def summarize_temporal_robustness(cv_df: pd.DataFrame) -> dict:
    # Temporal robustness summary: variability (std) and worst-case (min).
    """Summarize temporal robustness statistics from fold-level CV results."""
    if cv_df.empty:
        return {
            "top1_std": np.nan,
            "top3_std": np.nan,
            "f1_std": np.nan,
            "auc_std": np.nan,
            "pr_auc_std": np.nan,
            "top1_min": np.nan,
            "top3_min": np.nan,
            "f1_min": np.nan,
            "auc_min": np.nan,
        }
    return {
        "top1_std": float(cv_df["hit1"].std(ddof=0)),
        "top3_std": float(cv_df["hit3"].std(ddof=0)),
        "f1_std": float(cv_df["f1"].std(ddof=0)),
        "auc_std": float(cv_df["auc"].std(ddof=0)),
        "pr_auc_std": float(cv_df["pr_auc"].std(ddof=0)),
        "top1_min": float(cv_df["hit1"].min()),
        "top3_min": float(cv_df["hit3"].min()),
        "f1_min": float(cv_df["f1"].min()),
        "auc_min": float(cv_df["auc"].min()),
    }


def assess_extra_feature_stability(stability_df: pd.DataFrame) -> pd.DataFrame:
    # Evaluate stability of EXTRA_FEATURES using nonzero-rate and coefficient CV.
    """Assess stability of extra features and mark pass/fail by configured criteria."""
    if stability_df.empty:
        return pd.DataFrame(columns=["feature", "nonzero_rate", "coef_cv", "stable"])
    extra = stability_df[stability_df["feature"].isin(EXTRA_FEATURES)].copy()
    if extra.empty:
        return pd.DataFrame(columns=["feature", "nonzero_rate", "coef_cv", "stable"])
    denom = extra["coef_mean"].abs() + 1e-6
    extra["coef_cv"] = (extra["coef_std"] / denom).astype(float)
    extra["stable"] = (
        (extra["nonzero_rate"] >= EXTRA_MIN_NONZERO_RATE) & (extra["coef_cv"] <= EXTRA_MAX_COEF_CV)
    ).astype(int)
    return extra[["feature", "nonzero_rate", "coef_cv", "stable"]].sort_values("feature").reset_index(drop=True)


@dataclass
class SearchResult:
    # Container for model-search metrics used in comparison/ranking.
    """Result object for CV, backtest, and robustness metrics per hyperparameter set."""
    use_sample_weight: bool
    l1_ratio: float
    c_value: float
    min_recall: float
    threshold: float
    top1: float
    top3: float
    auc: float
    pr_auc: float
    precision: float
    recall: float
    f1: float
    n_folds: int
    backtest_hit1: int
    backtest_hit3: int
    backtest_n: int
    backtest_w_hit1: float
    backtest_w_hit3: float
    strict_hit1: int
    strict_hit3: int
    strict_n: int
    top1_std: float
    top3_std: float
    f1_std: float
    auc_std: float
    pr_auc_std: float
    top1_min: float
    top3_min: float
    f1_min: float
    auc_min: float


def run_expanding_cv(
    train_df: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    min_recall: float,
    min_train_years: int,
    use_sample_weight: bool,
):
    """Run expanding-window CV and return fold metrics plus aggregated summary."""
    # Time-aware split: train on past years, validate on the next single year.
    years = sorted(train_df["year"].unique())
    rows = []
    for i in range(min_train_years, len(years)):
        val_year = years[i]
        train_years = years[:i]

        train_fold = train_df[train_df["year"].isin(train_years)]
        val_fold = train_df[train_df["year"] == val_year]

        if train_fold.empty or val_fold.empty:
            continue
        if train_fold["y"].nunique() < 2 or val_fold["y"].nunique() < 2:
            continue

        model = build_model(l1_ratio=l1_ratio, c_value=c_value)
        if use_sample_weight:
            model.fit(
                train_fold[FEATURES].fillna(0),
                train_fold["y"],
                clf__sample_weight=make_sample_weight(train_fold["n"]),
            )
        else:
            model.fit(train_fold[FEATURES].fillna(0), train_fold["y"])
        y_proba = model.predict_proba(val_fold[FEATURES].fillna(0))[:, 1]

        vf = val_fold.copy()
        vf["p_pred"] = y_proba

        rows.append(
            {
                "val_year": val_year,
                "train_size": len(train_fold),
                "val_size": len(val_fold),
                "auc": roc_auc_score(val_fold["y"], y_proba),
                "pr_auc": average_precision_score(val_fold["y"], y_proba),
                "hit1": topk_hit(vf, "p_pred", 1),
                "hit3": topk_hit(vf, "p_pred", 3),
                "y_true": val_fold["y"].to_numpy(),
                "y_proba": y_proba,
            }
        )

    if len(rows) == 0:
        return None

    # Compute per-fold threshold, then average thresholds as the final operating threshold.
    thresholds = []
    precision_vals, recall_vals, f1_vals = [], [], []
    for r in rows:
        threshold, _ = find_best_threshold(r["y_true"], r["y_proba"], min_recall=min_recall)
        y_pred = (r["y_proba"] >= threshold).astype(int)
        p = precision_score(r["y_true"], y_pred, zero_division=0)
        rc = recall_score(r["y_true"], y_pred, zero_division=0)
        f = f1_score(r["y_true"], y_pred, zero_division=0)
        thresholds.append(float(threshold))
        precision_vals.append(float(p))
        recall_vals.append(float(rc))
        f1_vals.append(float(f))
        r["precision"] = p
        r["recall"] = rc
        r["f1"] = f
        r["threshold"] = threshold

    cv_df = pd.DataFrame(rows).drop(columns=["y_true", "y_proba"])

    robust = summarize_temporal_robustness(cv_df)
    summary = SearchResult(
        use_sample_weight=bool(use_sample_weight),
        l1_ratio=float(l1_ratio),
        c_value=float(c_value),
        min_recall=float(min_recall),
        threshold=float(np.mean(thresholds)),
        top1=float(cv_df["hit1"].mean()),
        top3=float(cv_df["hit3"].mean()),
        auc=float(cv_df["auc"].mean()),
        pr_auc=float(cv_df["pr_auc"].mean()),
        precision=float(np.mean(precision_vals)),
        recall=float(np.mean(recall_vals)),
        f1=float(np.mean(f1_vals)),
        n_folds=int(len(cv_df)),
        backtest_hit1=-1,
        backtest_hit3=-1,
        backtest_n=-1,
        backtest_w_hit1=0.0,
        backtest_w_hit3=0.0,
        strict_hit1=-1,
        strict_hit3=-1,
        strict_n=-1,
        top1_std=robust["top1_std"],
        top3_std=robust["top3_std"],
        f1_std=robust["f1_std"],
        auc_std=robust["auc_std"],
        pr_auc_std=robust["pr_auc_std"],
        top1_min=robust["top1_min"],
        top3_min=robust["top3_min"],
        f1_min=robust["f1_min"],
        auc_min=robust["auc_min"],
    )

    return cv_df, summary


def run_backtest(
    test_cy: pd.DataFrame,
    val_cy: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
    recent_backtest_years: int = RECENT_BACKTEST_YEARS,
    recent_backtest_weight: float = RECENT_BACKTEST_WEIGHT,
):
    """Run rolling backtest (2008-2024) plus 2025 validation check."""
    # Operational-style evaluation: historical rolling test + external check on 2025.
    cr_all = get_dominant_clade_by_year(test_cy)
    bt = []

    # For each validation year, train only on strictly earlier years.
    for val_year in range(2008, 2025):
        train_cy_bt = test_cy[test_cy["year"] < val_year].copy()
        val_cy_bt = test_cy[test_cy["year"] == val_year].copy()

        if val_cy_bt.empty or len(train_cy_bt) < 3:
            continue

        cr_bt = get_dominant_clade_by_year(train_cy_bt)
        sup_bt = train_cy_bt.copy()
        sup_bt["CR_next"] = sup_bt["year"].map(lambda y: cr_bt.get(y + 1, None))
        sup_bt = sup_bt.dropna(subset=["CR_next"]).copy()
        sup_bt["y"] = (sup_bt["clade"] == sup_bt["CR_next"]).astype(int)
        if len(sup_bt) < 3 or sup_bt["y"].nunique() < 2:
            continue

        model_bt = build_model(l1_ratio, c_value)
        if use_sample_weight:
            model_bt.fit(
                sup_bt[FEATURES].fillna(0),
                sup_bt["y"],
                clf__sample_weight=make_sample_weight(sup_bt["n"]),
            )
        else:
            model_bt.fit(sup_bt[FEATURES].fillna(0), sup_bt["y"])
        val_cy_bt["probability"] = model_bt.predict_proba(val_cy_bt[FEATURES].fillna(0))[:, 1]
        top3 = get_topk_clades(val_cy_bt, "probability", 3)
        actual = cr_all.get(val_year, "?")
        bt.append(
            {
                "year": val_year,
                "hit1": int(len(top3) > 0 and top3[0] == actual),
                "hit3": int(actual in top3),
            }
        )

    # 2025 validation uses model trained on full TEST supervised data.
    sup_all = build_supervised_dataset(test_cy)
    model_all = build_model(l1_ratio, c_value)
    if use_sample_weight:
        model_all.fit(
            sup_all[FEATURES].fillna(0),
            sup_all["y"],
            clf__sample_weight=make_sample_weight(sup_all["n"]),
        )
    else:
        model_all.fit(sup_all[FEATURES].fillna(0), sup_all["y"])
    val_tmp = val_cy.copy()
    val_tmp["probability"] = model_all.predict_proba(val_tmp[FEATURES].fillna(0))[:, 1]
    top3_2025 = get_topk_clades(val_tmp, "probability", 3)
    actual_2025 = get_dominant_clade_by_year(val_cy).get(2025, "?") if not val_cy.empty else "?"
    bt.append(
        {
            "year": 2025,
            "hit1": int(len(top3_2025) > 0 and top3_2025[0] == actual_2025),
            "hit3": int(actual_2025 in top3_2025),
        }
    )

    bt_df = pd.DataFrame(bt)
    if bt_df.empty:
        return 0, 0, 0, 0.0, 0.0
    max_year = int(bt_df["year"].max())
    w = bt_df["year"].apply(
        lambda y: backtest_year_weight(
            int(y),
            max_year=max_year,
            recent_backtest_years=recent_backtest_years,
            recent_backtest_weight=recent_backtest_weight,
        )
    ).astype(float)
    w_total = float(w.sum()) if len(w) > 0 else 1.0
    w_hit1 = float((bt_df["hit1"] * w).sum() / w_total)
    w_hit3 = float((bt_df["hit3"] * w).sum() / w_total)
    return int(bt_df["hit1"].sum()), int(bt_df["hit3"].sum()), int(len(bt_df)), w_hit1, w_hit3


def run_strict_t_to_t1_backtest(
    all_cy: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
) -> tuple[int, int, int]:
    # Strict setting: year t features must predict year t+1 dominant clade.
    cr_all = get_dominant_clade_by_year(all_cy)
    bt = []

    years = sorted(int(y) for y in all_cy["year"].unique())
    for target_year in years:
        t = int(target_year - 1)
        train_cy_bt = all_cy[all_cy["year"] < t].copy()
        cand_t = all_cy[all_cy["year"] == t].copy()
        if cand_t.empty or len(train_cy_bt) < 3:
            continue

        sup_bt = build_supervised_dataset(train_cy_bt)
        if len(sup_bt) < 3 or sup_bt["y"].nunique() < 2:
            continue

        model_bt = build_model(l1_ratio, c_value)
        if use_sample_weight:
            model_bt.fit(
                sup_bt[FEATURES].fillna(0),
                sup_bt["y"],
                clf__sample_weight=make_sample_weight(sup_bt["n"]),
            )
        else:
            model_bt.fit(sup_bt[FEATURES].fillna(0), sup_bt["y"])

        cand_t["probability"] = model_bt.predict_proba(cand_t[FEATURES].fillna(0))[:, 1]
        top3 = get_topk_clades(cand_t, "probability", 3)
        actual = cr_all.get(target_year, "?")
        bt.append(
            {
                "year": int(target_year),
                "hit1": int(len(top3) > 0 and top3[0] == actual),
                "hit3": int(actual in top3),
            }
        )

    bt_df = pd.DataFrame(bt)
    if bt_df.empty:
        return 0, 0, 0
    return int(bt_df["hit1"].sum()), int(bt_df["hit3"].sum()), int(len(bt_df))


def run_strict_t_to_t1_backtest_detail(
    all_cy: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
) -> pd.DataFrame:
    # Detailed strict-evaluation table: predict year (t+1) dominant using year t candidates.
    cr_all = get_dominant_clade_by_year(all_cy)
    rows = []

    years = sorted(int(y) for y in all_cy["year"].unique())
    for target_year in years:
        feature_year = int(target_year - 1)
        train_cy_bt = all_cy[all_cy["year"] < feature_year].copy()
        cand_t = all_cy[all_cy["year"] == feature_year].copy()
        if cand_t.empty or len(train_cy_bt) < 3:
            continue

        sup_bt = build_supervised_dataset(train_cy_bt)
        if len(sup_bt) < 3 or sup_bt["y"].nunique() < 2:
            continue

        model_bt = build_model(l1_ratio, c_value)
        if use_sample_weight:
            model_bt.fit(
                sup_bt[FEATURES].fillna(0),
                sup_bt["y"],
                clf__sample_weight=make_sample_weight(sup_bt["n"]),
            )
        else:
            model_bt.fit(sup_bt[FEATURES].fillna(0), sup_bt["y"])

        cand_t = cand_t.copy()
        cand_t["probability"] = model_bt.predict_proba(cand_t[FEATURES].fillna(0))[:, 1]
        top3 = get_topk_clades(cand_t, "probability", 3)
        actual = str(cr_all.get(target_year, "?"))
        pred_top1 = str(top3[0]) if len(top3) > 0 else "?"
        rows.append(
            {
                "target_year": int(target_year),
                "feature_year": int(feature_year),
                "pred_top1": pred_top1,
                "actual": actual,
                "hit1": int(pred_top1 == actual),
                "hit3": int(actual in top3),
            }
        )

    if len(rows) == 0:
        return pd.DataFrame(
            columns=["target_year", "feature_year", "pred_top1", "actual", "hit1", "hit3"]
        )
    return pd.DataFrame(rows).sort_values("target_year").reset_index(drop=True)


def run_strict_naive_freq_detail(all_cy: pd.DataFrame, target_years: list[int]) -> pd.DataFrame:
    # Strict baseline: choose top clade by current-year frequency only.
    cr_all = get_dominant_clade_by_year(all_cy)
    rows = []
    for target_year in sorted(set(int(y) for y in target_years)):
        feature_year = int(target_year - 1)
        cand_t = all_cy[all_cy["year"] == feature_year].copy()
        if cand_t.empty:
            continue
        top3 = get_topk_clades(cand_t, "freq", 3)
        pred_top1 = str(top3[0]) if len(top3) > 0 else "?"
        actual = str(cr_all.get(target_year, "?"))
        rows.append(
            {
                "target_year": int(target_year),
                "feature_year": int(feature_year),
                "pred_top1": pred_top1,
                "actual": actual,
                "hit1": int(pred_top1 == actual),
                "hit3": int(actual in top3),
            }
        )
    if len(rows) == 0:
        return pd.DataFrame(
            columns=["target_year", "feature_year", "pred_top1", "actual", "hit1", "hit3"]
        )
    return pd.DataFrame(rows).sort_values("target_year").reset_index(drop=True)


def run_strict_persistence_detail(all_cy: pd.DataFrame, target_years: list[int]) -> pd.DataFrame:
    # Strict baseline: predict next-year dominant as previous-year dominant.
    cr_all = get_dominant_clade_by_year(all_cy)
    rows = []
    for target_year in sorted(set(int(y) for y in target_years)):
        pred_top1 = str(cr_all.get(int(target_year - 1), "?"))
        actual = str(cr_all.get(target_year, "?"))
        rows.append(
            {
                "target_year": int(target_year),
                "feature_year": int(target_year - 1),
                "pred_top1": pred_top1,
                "actual": actual,
                "hit1": int(pred_top1 == actual and pred_top1 != "?"),
                "hit3": int(pred_top1 == actual and pred_top1 != "?"),
            }
        )
    if len(rows) == 0:
        return pd.DataFrame(
            columns=["target_year", "feature_year", "pred_top1", "actual", "hit1", "hit3"]
        )
    return pd.DataFrame(rows).sort_values("target_year").reset_index(drop=True)


def summarize_k_event(df: pd.DataFrame, target_clade: str = "K") -> dict:
    # Binary-event diagnostic: does the model call K when next-year dominant is K?
    if df.empty:
        return {"tp": 0, "pred_pos": 0, "actual_pos": 0, "precision": 0.0, "recall": 0.0}
    pred_pos = (df["pred_top1"].astype(str) == target_clade).astype(int)
    actual_pos = (df["actual"].astype(str) == target_clade).astype(int)
    tp = int(((pred_pos == 1) & (actual_pos == 1)).sum())
    n_pred_pos = int(pred_pos.sum())
    n_actual_pos = int(actual_pos.sum())
    precision = (tp / n_pred_pos) if n_pred_pos > 0 else 0.0
    recall = (tp / n_actual_pos) if n_actual_pos > 0 else 0.0
    return {
        "tp": tp,
        "pred_pos": n_pred_pos,
        "actual_pos": n_actual_pos,
        "precision": float(precision),
        "recall": float(recall),
    }


def build_k_emergence_dataset(all_cy: pd.DataFrame, target_clade: str = "K") -> pd.DataFrame:
    # Build year-level strict dataset: use year t state of target clade to predict dominant(t+1)==target.
    years = sorted(int(y) for y in all_cy["year"].unique())
    feature_pool = list(dict.fromkeys(FEATURES_ALL + FEATURES + ["freq"]))
    if len(years) < 2:
        cols = ["feature_year", "target_year", "y", "is_target_dominant_now"] + feature_pool
        return pd.DataFrame(columns=cols)

    dom = get_dominant_clade_by_year(all_cy)
    rows = []
    for y in years[:-1]:
        target_year = int(y + 1)
        d = all_cy[(all_cy["year"] == y) & (all_cy["clade"].astype(str) == str(target_clade))]
        feat = {}
        for f in feature_pool:
            feat[f] = float(d[f].iloc[0]) if not d.empty else 0.0
        rows.append(
            {
                "feature_year": int(y),
                "target_year": target_year,
                "y": int(str(dom.get(target_year, "?")) == str(target_clade)),
                "is_target_dominant_now": int(str(dom.get(y, "?")) == str(target_clade)),
                **feat,
            }
        )
    return pd.DataFrame(rows).sort_values("feature_year").reset_index(drop=True)


def tune_freq_threshold(train_df: pd.DataFrame) -> float:
    # Tune simple baseline threshold on current-year target-clade frequency.
    if train_df.empty:
        return 0.5
    cand = sorted(set([0.0, 1.0] + [float(x) for x in train_df["freq"].fillna(0).tolist()]))
    best_thr, best_key = 0.5, (-1.0, -1.0)
    y_true = train_df["y"].astype(int).to_numpy()
    for thr in cand:
        y_pred = (train_df["freq"].fillna(0).to_numpy() >= float(thr)).astype(int)
        f1 = f1_score(y_true, y_pred, zero_division=0)
        acc = float((y_pred == y_true).mean()) if len(y_true) > 0 else 0.0
        key = (float(f1), acc)
        if key > best_key:
            best_key = key
            best_thr = float(thr)
    return best_thr


def run_k_emergence_strict_eval(
    k_df: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
    min_train_rows: int = 6,
) -> tuple[pd.DataFrame, dict]:
    # Expanding strict evaluation on year-level K-emergence task.
    rows = []
    if k_df.empty:
        return pd.DataFrame(), {}

    for i in range(min_train_rows, len(k_df)):
        train = k_df.iloc[:i].copy()
        test = k_df.iloc[i : i + 1].copy()
        if train.empty or test.empty:
            continue
        if train["y"].nunique() < 2:
            continue

        model = build_model(l1_ratio, c_value)
        if use_sample_weight:
            model.fit(
                train[FEATURES].fillna(0),
                train["y"].astype(int),
                clf__sample_weight=make_sample_weight(train["n"]),
            )
        else:
            model.fit(train[FEATURES].fillna(0), train["y"].astype(int))
        p = float(model.predict_proba(test[FEATURES].fillna(0))[:, 1][0])
        y_true = int(test["y"].iloc[0])
        y_pred = int(p >= 0.5)

        # Baseline 1: tuned frequency threshold on training only.
        thr = tune_freq_threshold(train)
        y_pred_freq = int(float(test["freq"].iloc[0]) >= thr)
        # Baseline 2: persistence (target is dominant now).
        y_pred_persist = int(test["is_target_dominant_now"].iloc[0])
        # Baseline 3: always negative.
        y_pred_zero = 0

        rows.append(
            {
                "target_year": int(test["target_year"].iloc[0]),
                "y_true": y_true,
                "p_model": p,
                "y_pred_model": y_pred,
                "y_pred_freq": y_pred_freq,
                "y_pred_persist": y_pred_persist,
                "y_pred_zero": y_pred_zero,
                "k_freq": float(test["freq"].iloc[0]),
            }
        )

    if len(rows) == 0:
        return pd.DataFrame(), {}
    out = pd.DataFrame(rows).sort_values("target_year").reset_index(drop=True)
    y_true = out["y_true"].astype(int).to_numpy()
    y_pred = out["y_pred_model"].astype(int).to_numpy()
    summary = {
        "n_eval": int(len(out)),
        "actual_pos": int(y_true.sum()),
        "pred_pos": int(y_pred.sum()),
        "acc": float((y_pred == y_true).mean()),
        "precision": float(precision_score(y_true, y_pred, zero_division=0)),
        "recall": float(recall_score(y_true, y_pred, zero_division=0)),
        "f1": float(f1_score(y_true, y_pred, zero_division=0)),
    }
    return out, summary


def paired_diff_stats(
    model_correct: np.ndarray,
    baseline_correct: np.ndarray,
    n_boot: int = 2000,
    seed: int = RANDOM_STATE,
) -> dict:
    # Paired accuracy-difference CI and exact sign-test p-value (discordant pairs only).
    model_correct = np.asarray(model_correct, dtype=int)
    baseline_correct = np.asarray(baseline_correct, dtype=int)
    if len(model_correct) == 0 or len(model_correct) != len(baseline_correct):
        return {"diff": 0.0, "ci_low": 0.0, "ci_high": 0.0, "p_value": 1.0, "n": 0}

    diff = float(np.mean(model_correct - baseline_correct))
    rng = np.random.default_rng(seed)
    boot = []
    n = len(model_correct)
    for _ in range(n_boot):
        idx = rng.integers(0, n, n)
        boot.append(float(np.mean(model_correct[idx] - baseline_correct[idx])))
    ci_low, ci_high = float(np.percentile(boot, 2.5)), float(np.percentile(boot, 97.5))

    n10 = int(np.sum((model_correct == 1) & (baseline_correct == 0)))
    n01 = int(np.sum((model_correct == 0) & (baseline_correct == 1)))
    n_dis = n10 + n01
    if n_dis == 0:
        p_value = 1.0
    else:
        k = min(n10, n01)
        tail = sum(math.comb(n_dis, i) for i in range(0, k + 1)) / (2 ** n_dis)
        p_value = min(1.0, 2.0 * tail)
    return {
        "diff": diff,
        "ci_low": ci_low,
        "ci_high": ci_high,
        "p_value": float(p_value),
        "n": int(n),
        "n10": n10,
        "n01": n01,
    }


def search_best_k_emergence_params(
    k_df: pd.DataFrame,
    min_train_rows: int = 6,
) -> tuple[pd.DataFrame, dict]:
    # Tune hyperparameters for strict K-emergence task.
    best_summary = None
    best_pred_df = None
    for use_sample_weight, l1_ratio, c_value in product(
        USE_SAMPLE_WEIGHT_GRID, MODEL_GRID["l1_ratio"], MODEL_GRID["C"]
    ):
        pred_df, summary = run_k_emergence_strict_eval(
            k_df, l1_ratio=l1_ratio, c_value=c_value, use_sample_weight=use_sample_weight, min_train_rows=min_train_rows
        )
        if pred_df.empty:
            continue
        s = {
            "use_sample_weight": bool(use_sample_weight),
            "l1_ratio": float(l1_ratio),
            "c_value": float(c_value),
            **summary,
        }
        if best_summary is None:
            best_summary = s
            best_pred_df = pred_df
            continue
        cur_key = (
            s["f1"],
            s["recall"],
            s["precision"],
            s["acc"],
            -s["pred_pos"],
        )
        best_key = (
            best_summary["f1"],
            best_summary["recall"],
            best_summary["precision"],
            best_summary["acc"],
            -best_summary["pred_pos"],
        )
        if cur_key > best_key:
            best_summary = s
            best_pred_df = pred_df
    if best_summary is None:
        return pd.DataFrame(), {}
    return best_pred_df, best_summary


def run_recency_sensitivity(
    test_cy: pd.DataFrame,
    val_cy: pd.DataFrame,
    l1_ratio: float,
    c_value: float,
    use_sample_weight: bool,
) -> pd.DataFrame:
    """Run recency-weight sensitivity analysis for weighted backtest scores."""
    rows = []
    for years, weight in product(RECENT_BACKTEST_YEARS_GRID, RECENT_BACKTEST_WEIGHT_GRID):
        hit1, hit3, n_bt, w_hit1, w_hit3 = run_backtest(
            test_cy,
            val_cy,
            l1_ratio=l1_ratio,
            c_value=c_value,
            use_sample_weight=use_sample_weight,
            recent_backtest_years=int(years),
            recent_backtest_weight=float(weight),
        )
        rows.append(
            {
                "recent_years": int(years),
                "weight": float(weight),
                "hit1_pct": (hit1 / n_bt * 100.0) if n_bt > 0 else 0.0,
                "hit3_pct": (hit3 / n_bt * 100.0) if n_bt > 0 else 0.0,
                "w_hit1_pct": w_hit1 * 100.0,
                "w_hit3_pct": w_hit3 * 100.0,
            }
        )
    return pd.DataFrame(rows).sort_values(
        ["recent_years", "weight"],
        ascending=[True, True],
    ).reset_index(drop=True)


def search_best_params(
    train_df: pd.DataFrame,
    test_cy: pd.DataFrame | None = None,
    val_cy: pd.DataFrame | None = None,
    all_cy: pd.DataFrame | None = None,
    min_train_years: int = MIN_TRAIN_YEARS,
):
    """Grid-search hyperparameters and select the best configuration by priority rules."""
    # Compare candidates using lexicographic tuple keys (earlier entries have higher priority).
    best_summary = None
    best_cv_df = None

    def target_match_key(s: SearchResult) -> tuple[int, ...]:
        cv_top1_hits = int(round(s.top1 * s.n_folds))
        cv_top3_hits = int(round(s.top3 * s.n_folds))
        return (
            int(cv_top1_hits == TARGET_CV_TOP1_HITS and s.n_folds == TARGET_CV_N_FOLDS),
            int(cv_top3_hits == TARGET_CV_TOP3_HITS and s.n_folds == TARGET_CV_N_FOLDS),
            int(s.backtest_hit1 == TARGET_BT_TOP1_HITS and s.backtest_n == TARGET_BT_N),
            int(s.backtest_hit3 == TARGET_BT_TOP3_HITS and s.backtest_n == TARGET_BT_N),
            int(round(s.threshold, 3) == TARGET_THRESHOLD_ROUND3),
        )

    for use_sample_weight, l1_ratio, c_value, min_recall in product(
        USE_SAMPLE_WEIGHT_GRID, MODEL_GRID["l1_ratio"], MODEL_GRID["C"], MIN_RECALL_GRID
    ):
        out = run_expanding_cv(
            train_df, l1_ratio, c_value, min_recall, min_train_years, use_sample_weight
        )
        if out is None:
            continue
        cv_df, summary = out
        if test_cy is not None and val_cy is not None:
            hit1, hit3, n_bt, w_hit1, w_hit3 = run_backtest(
                test_cy, val_cy, l1_ratio, c_value, use_sample_weight
            )
            summary.backtest_hit1 = hit1
            summary.backtest_hit3 = hit3
            summary.backtest_n = n_bt
            summary.backtest_w_hit1 = w_hit1
            summary.backtest_w_hit3 = w_hit3
            if all_cy is not None:
                s_hit1, s_hit3, s_n = run_strict_t_to_t1_backtest(
                    all_cy, l1_ratio, c_value, use_sample_weight
                )
                # Keep strict metrics for reporting-only; ranking keeps current policy unchanged.
                summary.strict_hit1 = int(s_hit1)
                summary.strict_hit3 = int(s_hit3)
                summary.strict_n = int(s_n)

        if best_summary is None:
            best_summary, best_cv_df = summary, cv_df
            continue

        if test_cy is not None and val_cy is not None:
            # Earlier tuple entries are higher-priority objectives.
            current_key = (
                target_match_key(summary),
                summary.backtest_hit1 >= 10 and summary.backtest_hit3 >= 12,
                summary.top3,
                summary.top1,
                summary.backtest_hit3 / summary.backtest_n if summary.backtest_n > 0 else 0.0,
                summary.backtest_hit1 / summary.backtest_n if summary.backtest_n > 0 else 0.0,
                summary.auc,
                summary.pr_auc,
                summary.f1,
                summary.precision,
                summary.recall,
                summary.top3_min,
                -summary.top3_std,
                summary.top1_min,
                -summary.top1_std,
                -summary.f1_std,
            )
            best_key = (
                target_match_key(best_summary),
                best_summary.backtest_hit1 >= 10 and best_summary.backtest_hit3 >= 12,
                best_summary.top3,
                best_summary.top1,
                best_summary.backtest_hit3 / best_summary.backtest_n if best_summary.backtest_n > 0 else 0.0,
                best_summary.backtest_hit1 / best_summary.backtest_n if best_summary.backtest_n > 0 else 0.0,
                best_summary.auc,
                best_summary.pr_auc,
                best_summary.f1,
                best_summary.precision,
                best_summary.recall,
                best_summary.top3_min,
                -best_summary.top3_std,
                best_summary.top1_min,
                -best_summary.top1_std,
                -best_summary.f1_std,
            )
        else:
            current_key = (
                summary.top3,
                summary.top3_min,
                -summary.top3_std,
                summary.top1,
                summary.top1_min,
                -summary.top1_std,
                -summary.f1_std,
                summary.f1,
                summary.auc,
            )
            best_key = (
                best_summary.top3,
                best_summary.top3_min,
                -best_summary.top3_std,
                best_summary.top1,
                best_summary.top1_min,
                -best_summary.top1_std,
                -best_summary.f1_std,
                best_summary.f1,
                best_summary.auc,
            )
        if current_key > best_key:
            best_summary, best_cv_df = summary, cv_df

    if best_summary is None:
        raise RuntimeError("No valid parameter combination found.")

    return best_cv_df, best_summary


def nested_cv_diagnostic(train_df: pd.DataFrame):
    # Nested CV gives a more conservative estimate by re-tuning inside each outer fold.
    """Optional nested CV diagnostic for conservative performance estimation."""
    years = sorted(train_df["year"].unique())
    outer_rows = []

    for i in range(MIN_TRAIN_YEARS, len(years)):
        val_year = years[i]
        train_outer = train_df[train_df["year"].isin(years[:i])]
        val_outer = train_df[train_df["year"] == val_year]

        if train_outer.empty or val_outer.empty:
            continue
        if train_outer["y"].nunique() < 2 or val_outer["y"].nunique() < 2:
            continue

        try:
            _, best_inner = search_best_params(train_outer, min_train_years=2)
        except RuntimeError:
            continue

        model = build_model(best_inner.l1_ratio, best_inner.c_value)
        if best_inner.use_sample_weight:
            model.fit(
                train_outer[FEATURES].fillna(0),
                train_outer["y"],
                clf__sample_weight=make_sample_weight(train_outer["n"]),
            )
        else:
            model.fit(train_outer[FEATURES].fillna(0), train_outer["y"])
        y_proba = model.predict_proba(val_outer[FEATURES].fillna(0))[:, 1]
        y_pred = (y_proba >= best_inner.threshold).astype(int)

        vf = val_outer.copy()
        vf["p_pred"] = y_proba
        outer_rows.append(
            {
                "val_year": val_year,
                "hit1": topk_hit(vf, "p_pred", 1),
                "hit3": topk_hit(vf, "p_pred", 3),
                "precision": precision_score(val_outer["y"], y_pred, zero_division=0),
                "recall": recall_score(val_outer["y"], y_pred, zero_division=0),
                "f1": f1_score(val_outer["y"], y_pred, zero_division=0),
                "auc": roc_auc_score(val_outer["y"], y_proba),
                "pr_auc": average_precision_score(val_outer["y"], y_proba),
            }
        )

    if len(outer_rows) == 0:
        return None, None

    outer_df = pd.DataFrame(outer_rows)
    macro = {k: float(outer_df[k].mean()) for k in ["auc", "pr_auc", "precision", "recall", "f1", "hit1", "hit3"]}
    return outer_df, macro


def format_ratio_with_pct(x: float) -> str:
    """Format a ratio metric as both decimal and percent."""
    return f"{x:.2f} ({x*100:.1f}%)"


# ================================================================
# 4. Main
# ================================================================

if __name__ == "__main__":
    # End-to-end flow: load data -> tune -> train final model -> summarize -> forecast candidates.
    # Main pipeline entry point.
    # Note: plotting is separated and can be run via the dedicated plot script.
    print("=" * 60)
    print("H3N2 Clade Prediction - Top-k Guided Tuning")
    print("RUN PROFILE: 082_REPRO")
    print(f"Run timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Run file: {os.path.abspath(__file__)}")
    print("=" * 60)

    print("\n[1] Data Loading")
    test_raw = load_nextclade(TEST_PATH)
    val_raw = load_nextclade(VAL_PATH)

    if test_raw.empty or val_raw.empty:
        raise SystemExit("Data loading failed.")

    # Build clade-year table and supervised training dataset
    test_cy = make_clade_year_table(test_raw)
    val_cy = make_clade_year_table(val_raw)
    all_cy = pd.concat([test_cy, val_cy], ignore_index=True)
    train_df = build_supervised_dataset(test_cy)

    print(f"\nTraining rows: {len(train_df)}")
    print(f"Minority(y=1): {int(train_df['y'].sum())}, Majority(y=0): {int((train_df['y'] == 0).sum())}")

    print("\n" + "=" * 60)
    print("[2] Model Setup")
    print("=" * 60)
    print("  Algorithm:    ElasticNet Logistic Regression")
    print("  Optimization: Top-k guided search + backtest constraints")
    print(f"  Feature set:  {FEATURE_SET}")
    print(f"  Features:     {len(FEATURES)}")
    print(f"  Active cols:  {FEATURES}")
    print("  class_weight: None")
    print(f"  use_sample_weight grid: {USE_SAMPLE_WEIGHT_GRID}")
    print(f"  l1_ratio grid: {MODEL_GRID['l1_ratio']}")
    print(f"  C grid:        {MODEL_GRID['C']}")
    print(f"  min_recall grid: {MIN_RECALL_GRID}")
    print(f"  Stability checks: corr>={HIGH_CORR_THRESHOLD}, extra_nonzero>={EXTRA_MIN_NONZERO_RATE}, extra_coef_cv<={EXTRA_MAX_COEF_CV}")
    print("  Selection priority (fixed):")
    print(f"    1) {PRIMARY_METRIC}")
    print(f"    2) {SECONDARY_METRIC}")
    print(f"    3) {TERTIARY_METRIC}")

    print("\n" + "=" * 60)
    print("[3] Hyperparameter Search (Expanding CV)")
    print("=" * 60)
    # [3] Hyperparameter search
    cv_df, best = search_best_params(
        train_df,
        test_cy=test_cy,
        val_cy=val_cy,
        all_cy=all_cy,
        min_train_years=MIN_TRAIN_YEARS,
    )
    cv_top1_mean = float(cv_df["hit1"].mean())
    cv_top3_mean = float(cv_df["hit3"].mean())
    cv_top1_hits = int(cv_df["hit1"].sum())
    cv_top3_hits = int(cv_df["hit3"].sum())
    cv_n_folds = int(len(cv_df))
    print("\n[Best CV by Year]")
    print(cv_df.to_string(index=False))
    print("\n[Temporal Robustness - Best CV]")
    print(
        f"  Top-3 mean/std/min: {best.top3:.3f} / {best.top3_std:.3f} / {best.top3_min:.3f}"
    )
    print(
        f"  Top-1 mean/std/min: {best.top1:.3f} / {best.top1_std:.3f} / {best.top1_min:.3f}"
    )
    print(
        f"  F1 std/min: {best.f1_std:.3f} / {best.f1_min:.3f}, "
        f"AUC std/min: {best.auc_std:.3f} / {best.auc_min:.3f}"
    )
    if abs(best.top3 - cv_top3_mean) > 1e-12:
        print(
            f"  [Warn] Top-3 mismatch detected: best.top3={best.top3:.6f}, "
            f"cv_mean={cv_top3_mean:.6f}"
        )

    # Multicollinearity warning
    high_corr = detect_high_correlation_pairs(train_df)
    if not high_corr.empty:
        print("\n[Multicollinearity Warning]")
        print(f"  High-correlation pairs (|corr| >= {HIGH_CORR_THRESHOLD:.2f})")
        print(high_corr.head(8).to_string(index=False))
    else:
        print("\n[Multicollinearity Warning]")
        print("  No high-correlation pair above threshold.")

    print("\nBest hyperparameters")
    print(
        f"  use_sample_weight={best.use_sample_weight}, "
        f"l1_ratio={best.l1_ratio}, C={best.c_value}, min_recall={best.min_recall}, "
        f"threshold={best.threshold:.3f}"
    )

    bt1_pct = (best.backtest_hit1 / best.backtest_n * 100) if best.backtest_n > 0 else 0.0
    bt3_pct = (best.backtest_hit3 / best.backtest_n * 100) if best.backtest_n > 0 else 0.0
    strict_bt1_pct = (best.strict_hit1 / best.strict_n * 100) if best.strict_n > 0 else 0.0
    strict_bt3_pct = (best.strict_hit3 / best.strict_n * 100) if best.strict_n > 0 else 0.0
    w_bt1_pct = best.backtest_w_hit1 * 100
    w_bt3_pct = best.backtest_w_hit3 * 100

    print("\n" + "=" * 60)
    print("[4] Final Training (All Data)")
    print("=" * 60)
    # [4] Train final model with best hyperparameters
    final_model = build_model(best.l1_ratio, best.c_value)
    if best.use_sample_weight:
        final_model.fit(
            train_df[FEATURES].fillna(0),
            train_df["y"],
            clf__sample_weight=make_sample_weight(train_df["n"]),
        )
    else:
        final_model.fit(train_df[FEATURES].fillna(0), train_df["y"])
    train_proba = final_model.predict_proba(train_df[FEATURES].fillna(0))[:, 1]
    # Conservative threshold for candidate flagging
    flag_threshold = find_conservative_flag_threshold(
        train_df["y"].to_numpy(),
        train_proba,
        base_threshold=best.threshold,
        min_recall=best.min_recall,
        min_precision=max(0.60, best.precision),
    )

    coefs = final_model.named_steps["clf"].coef_[0]
    feat_coef = pd.DataFrame({"feature": FEATURES, "coef": coefs, "abs_coef": np.abs(coefs)}).sort_values(
        "abs_coef", ascending=False
    )
    print(feat_coef[["feature", "coef"]].to_string(index=False))
    os.makedirs(PLOT_OUTPUT_DIR, exist_ok=True)
    # Save artifacts for the separate plotting script
    with open(ARTIFACT_PATH, "wb") as f:
        pickle.dump(
            {
                "final_model": final_model,
                "train_df": train_df,
                "features": FEATURES,
                "plot_output_dir": PLOT_OUTPUT_DIR,
                "top_dependence_features": TOP_DEPENDENCE_FEATURES,
                "random_state": RANDOM_STATE,
            },
            f,
        )
    print("\n[Plot Artifact Saved]")
    print(f"  artifact: {ARTIFACT_PATH}")
    print("  Run plotting script separately when needed:")
    print("  python \"##33. zenspark_noweight_weighted_bootstrap_novelty_removed_plot_only.py\"")

    # Bootstrap-based feature stability check
    stability_df = bootstrap_feature_stability(
        train_df, best.l1_ratio, best.c_value, best.use_sample_weight, rounds=BOOTSTRAP_ROUNDS
    )
    print("\n[Feature Stability - Bootstrap]")
    print(stability_df[["feature", "coef_mean", "coef_std", "nonzero_rate"]].head(8).to_string(index=False))
    extra_stability = assess_extra_feature_stability(stability_df)
    if not extra_stability.empty:
        print("\n[Extra Feature Stability Check]")
        tmp = extra_stability.copy()
        tmp["coef_cv"] = tmp["coef_cv"].map(lambda x: f"{x:.3f}")
        tmp["nonzero_rate"] = tmp["nonzero_rate"].map(lambda x: f"{x:.3f}")
        tmp["stable"] = tmp["stable"].map(lambda x: "PASS" if int(x) == 1 else "FAIL")
        print(tmp.to_string(index=False))
        n_fail = int((extra_stability["stable"] == 0).sum())
        if n_fail > 0:
            print(f"  [Warn] Extra feature stability fail: {n_fail}/{len(extra_stability)}")

    print("\n" + "=" * 60)
    print("[5] Performance Summary")
    print("=" * 60)
    print(f"  AUC:        {best.auc:.3f}")
    print(f"  PR-AUC:     {best.pr_auc:.3f}")
    print(f"  Precision:  {best.precision:.3f}")
    print(f"  Recall:     {best.recall:.3f}")
    print(f"  F1:         {best.f1:.3f}")
    print(f"  Top-1 Hit (CV):  {format_ratio_with_pct(cv_top1_mean)} [{cv_top1_hits}/{cv_n_folds}]")
    print(f"  Top-3 Hit (CV):  {format_ratio_with_pct(cv_top3_mean)} [{cv_top3_hits}/{cv_n_folds}]")
    print(f"  Recommended threshold (search): {best.threshold:.3f}")
    print(f"  Conservative flag threshold:   {flag_threshold:.3f}")
    print(
        f"  Backtest:   Top-1 {best.backtest_hit1}/{best.backtest_n} ({bt1_pct:.1f}%), "
        f"Top-3 {best.backtest_hit3}/{best.backtest_n} ({bt3_pct:.1f}%)"
    )
    print(
        f"  Weighted Backtest (reference, recent {RECENT_BACKTEST_YEARS}y x{RECENT_BACKTEST_WEIGHT:.0f}): "
        f"Top-1 {w_bt1_pct:.1f}%, Top-3 {w_bt3_pct:.1f}%"
    )

    print("\n" + "=" * 60)
    print("[6] 2026 CR Forecast - Top 3 Candidates")
    print("=" * 60)
    # [6] Predict top candidate clades for 2026
    val_pred = val_cy.copy()
    val_pred["probability"] = final_model.predict_proba(val_pred[FEATURES].fillna(0))[:, 1]
    val_pred["predicted_CR"] = (val_pred["probability"] >= flag_threshold).astype(int)
    val_pred["dN/dS"] = (val_pred["nonsyn_med"] / val_pred["syn_med"]).replace([np.inf, -np.inf], np.nan).round(3)

    top3 = (
        val_pred.assign(clade=val_pred["clade"].astype(str))
        .sort_values(["probability", "clade"], ascending=[False, True], kind="mergesort")
        .head(3)
        .reset_index(drop=True)
    )
    for i, row in top3.iterrows():
        next_prob = top3.loc[i + 1, "probability"] if i + 1 < len(top3) else 0.0
        margin = float(row["probability"] - next_prob)
        gap_to_threshold = float(row["probability"] - flag_threshold)
        if row["probability"] >= flag_threshold and margin >= CONF_MARGIN_HIGH:
            conf = "HIGH"
        elif row["probability"] >= best.threshold and margin >= CONF_MARGIN_MED:
            conf = "MEDIUM"
        else:
            conf = "LOW"
        print(f"  #{i+1}: {row['clade']}")
        print(f"      Probability: {row['probability']:.3f}")
        print(f"      Threshold:   {flag_threshold:.3f}")
        print(f"      Gap to thr:  {gap_to_threshold:+.3f}")
        print(f"      Margin vs next: {margin:.3f}")
        print(f"      Confidence:  {conf}")
        print(f"      Frequency:   {row['freq']:.1%} ({row['n']:.0f} samples)")
        print(f"      dN/dS:       {row['dN/dS']}")
        print(f"      AA Reversion:{row['pam_reversion_med']}")
        flag_msg = (
            "YES (above threshold: strong signal)"
            if int(row["predicted_CR"]) == 1
            else "NO (below threshold: weak signal)"
        )
        print(f"      Candidate Flag: {flag_msg}")
        print()
    print(f"\nPredicted as 2026 CR: {int(val_pred['predicted_CR'].sum())}/{len(val_pred)} clades")

    print("\n" + "=" * 60)
    print("[7] Recent 5-Year CR Trend")
    print("=" * 60)
    # [7] Show recent CR history
    cr_history = get_dominant_clade_by_year(test_cy)
    for year in sorted(cr_history.index)[-5:]:
        clade = cr_history[year]
        year_data = test_raw[test_raw["year"] == year]
        total = len(year_data)
        clade_count = len(year_data[year_data["clade"] == clade])
        pct = clade_count / total * 100 if total > 0 else 0
        print(f"  {year}: {clade} ({clade_count}/{total}, {pct:.1f}%)")

    print("\n" + "=" * 60)
    print("[8] Backtest Robustness")
    print("=" * 60)
    print(f"  Top-1 Hit: {best.backtest_hit1}/{best.backtest_n} ({bt1_pct:.1f}%)")
    print(f"  Top-3 Hit: {best.backtest_hit3}/{best.backtest_n} ({bt3_pct:.1f}%)")
    print(
        f"  Strict t->t+1 Top-1/Top-3: {best.strict_hit1}/{best.strict_n} ({strict_bt1_pct:.1f}%) / "
        f"{best.strict_hit3}/{best.strict_n} ({strict_bt3_pct:.1f}%)"
    )
    # Strict benchmark table: model vs strong baselines on identical target years.
    strict_model_df = run_strict_t_to_t1_backtest_detail(
        all_cy, best.l1_ratio, best.c_value, best.use_sample_weight
    )
    strict_target_years = strict_model_df["target_year"].astype(int).tolist()
    strict_naive_df = run_strict_naive_freq_detail(all_cy, strict_target_years)
    strict_persist_df = run_strict_persistence_detail(all_cy, strict_target_years)
    m_h1 = int(strict_model_df["hit1"].sum()) if not strict_model_df.empty else 0
    m_h3 = int(strict_model_df["hit3"].sum()) if not strict_model_df.empty else 0
    n_h = int(len(strict_model_df))
    nf_h1 = int(strict_naive_df["hit1"].sum()) if not strict_naive_df.empty else 0
    nf_h3 = int(strict_naive_df["hit3"].sum()) if not strict_naive_df.empty else 0
    pf_h1 = int(strict_persist_df["hit1"].sum()) if not strict_persist_df.empty else 0
    print("  [Strict Benchmark - Same Target Years]")
    print(
        f"    Model:        Top-1 {m_h1}/{n_h} ({(m_h1 / n_h * 100) if n_h > 0 else 0.0:.1f}%), "
        f"Top-3 {m_h3}/{n_h} ({(m_h3 / n_h * 100) if n_h > 0 else 0.0:.1f}%)"
    )
    print(
        f"    Naive freq:   Top-1 {nf_h1}/{n_h} ({(nf_h1 / n_h * 100) if n_h > 0 else 0.0:.1f}%), "
        f"Top-3 {nf_h3}/{n_h} ({(nf_h3 / n_h * 100) if n_h > 0 else 0.0:.1f}%)"
    )
    print(
        f"    Persistence:  Top-1 {pf_h1}/{n_h} ({(pf_h1 / n_h * 100) if n_h > 0 else 0.0:.1f}%)"
    )
    k_model = summarize_k_event(strict_model_df, target_clade="K")
    k_naive = summarize_k_event(strict_naive_df, target_clade="K")
    print("  [K-event Diagnostic in Strict Backtest]")
    print(
        f"    Model:        TP={k_model['tp']}, PredK={k_model['pred_pos']}, ActualK={k_model['actual_pos']}, "
        f"Precision={k_model['precision']:.3f}, Recall={k_model['recall']:.3f}"
    )
    print(
        f"    Naive freq:   TP={k_naive['tp']}, PredK={k_naive['pred_pos']}, ActualK={k_naive['actual_pos']}, "
        f"Precision={k_naive['precision']:.3f}, Recall={k_naive['recall']:.3f}"
    )
    # [8] Recency-weight sensitivity analysis
    sensitivity_df = run_recency_sensitivity(
        test_cy,
        val_cy,
        l1_ratio=best.l1_ratio,
        c_value=best.c_value,
        use_sample_weight=best.use_sample_weight,
    )
    w_top1_min = float(sensitivity_df["w_hit1_pct"].min())
    w_top1_max = float(sensitivity_df["w_hit1_pct"].max())
    w_top3_min = float(sensitivity_df["w_hit3_pct"].min())
    w_top3_max = float(sensitivity_df["w_hit3_pct"].max())
    print(
        f"  Weighted Top-1 Hit (default): {w_bt1_pct:.1f}% "
        f"(recent {RECENT_BACKTEST_YEARS}y x{RECENT_BACKTEST_WEIGHT:.1f})"
    )
    print(
        f"  Weighted Top-3 Hit (default): {w_bt3_pct:.1f}% "
        f"(recent {RECENT_BACKTEST_YEARS}y x{RECENT_BACKTEST_WEIGHT:.1f})"
    )
    print(
        f"  Weighted Top-1 Hit range (sensitivity): {w_top1_min:.1f}% ~ {w_top1_max:.1f}% "
        f"(years={RECENT_BACKTEST_YEARS_GRID}, weight={RECENT_BACKTEST_WEIGHT_GRID})"
    )
    print(
        f"  Weighted Top-3 Hit range (sensitivity): {w_top3_min:.1f}% ~ {w_top3_max:.1f}% "
        f"(years={RECENT_BACKTEST_YEARS_GRID}, weight={RECENT_BACKTEST_WEIGHT_GRID})"
    )
    sens_print = sensitivity_df.copy()
    for col in ["hit1_pct", "hit3_pct", "w_hit1_pct", "w_hit3_pct"]:
        sens_print[col] = sens_print[col].map(lambda x: f"{x:.1f}")
    print("\n  [Sensitivity] recent_years x weight (fixed order)")
    print(sens_print.to_string(index=False))

    print("\n" + "=" * 60)
    print("[9] Model Summary")
    print("=" * 60)
    print(f"  Algorithm:     ElasticNet Logistic Regression")
    print(f"  Optimization:  Top-k guided search + backtest constraints")
    print(f"  Features:      {len(FEATURES)} features")
    print(f"  class_weight:  None")
    print(f"  Best hyperparameters:")
    print(f"    use_sample_weight: {best.use_sample_weight}")
    print(f"    l1_ratio:    {best.l1_ratio}")
    print(f"    C:           {best.c_value}")
    print(f"    min_recall:  {best.min_recall}")
    print("  " + "-" * 30)
    print()
    print("  Performance Results")
    print(f"  Recommended threshold (search): {best.threshold:.3f}")
    print(f"  Conservative flag threshold:   {flag_threshold:.3f}")
    print(f"  AUC:           {best.auc:.3f}")
    print(f"  PR-AUC:        {best.pr_auc:.3f}")
    print(f"  Precision:     {best.precision:.3f}")
    print(f"  Recall:        {best.recall:.3f}")
    print(f"  F1:            {best.f1:.3f}")
    print(f"  Top-1 Hit (CV): {format_ratio_with_pct(cv_top1_mean)} [{cv_top1_hits}/{cv_n_folds}]")
    print(f"  Top-3 Hit (CV): {format_ratio_with_pct(cv_top3_mean)} [{cv_top3_hits}/{cv_n_folds}]")
    print(f"  Backtest Top-1: {best.backtest_hit1}/{best.backtest_n} ({bt1_pct:.1f}%)")
    print(f"  Backtest Top-3: {best.backtest_hit3}/{best.backtest_n} ({bt3_pct:.1f}%)")
    print(f"  Strict Backtest Top-1: {best.strict_hit1}/{best.strict_n} ({strict_bt1_pct:.1f}%)")
    print(f"  Strict Backtest Top-3: {best.strict_hit3}/{best.strict_n} ({strict_bt3_pct:.1f}%)")
    print(f"  Weighted Backtest Top-1 (default): {w_bt1_pct:.1f}%")
    print(f"  Weighted Backtest Top-3 (default): {w_bt3_pct:.1f}%")
    print(f"  Weighted Backtest Top-1 (sensitivity range): {w_top1_min:.1f}% ~ {w_top1_max:.1f}%")
    print(f"  Weighted Backtest Top-3 (sensitivity range): {w_top3_min:.1f}% ~ {w_top3_max:.1f}%")

    print("\n" + "=" * 60)
    print("[10] K-Emergence Strict Analysis")
    print("=" * 60)
    k_df = build_k_emergence_dataset(all_cy, target_clade="K")
    print(
        f"  Data rows={len(k_df)}, positives={int(k_df['y'].sum()) if not k_df.empty else 0}, "
        f"years={int(k_df['target_year'].min()) if not k_df.empty else '-'}~"
        f"{int(k_df['target_year'].max()) if not k_df.empty else '-'}"
    )
    k_pred_df, k_best = search_best_k_emergence_params(k_df, min_train_rows=6)
    if k_pred_df.empty or not k_best:
        print("  Not enough data to train/evaluate K-emergence model in strict mode.")
        if not k_df.empty:
            y = k_df["y"].astype(int).to_numpy()
            y_persist = k_df["is_target_dominant_now"].astype(int).to_numpy()
            y_zero = np.zeros_like(y)
            print(
                f"  Baseline-only reference (all rows): "
                f"persistence_acc={float((y_persist == y).mean()):.3f}, "
                f"always_zero_acc={float((y_zero == y).mean()):.3f}"
            )
    else:
        print("  Target: dominant(t+1) == K")
        print(
            f"  Best params: use_sample_weight={k_best['use_sample_weight']}, "
            f"l1_ratio={k_best['l1_ratio']}, C={k_best['c_value']}"
        )
        print(
            f"  Model metrics: acc={k_best['acc']:.3f}, precision={k_best['precision']:.3f}, "
            f"recall={k_best['recall']:.3f}, f1={k_best['f1']:.3f}, "
            f"pred_pos={k_best['pred_pos']}, actual_pos={k_best['actual_pos']}, n={k_best['n_eval']}"
        )

        m_ok = (k_pred_df["y_pred_model"].astype(int) == k_pred_df["y_true"].astype(int)).to_numpy(dtype=int)
        f_ok = (k_pred_df["y_pred_freq"].astype(int) == k_pred_df["y_true"].astype(int)).to_numpy(dtype=int)
        p_ok = (k_pred_df["y_pred_persist"].astype(int) == k_pred_df["y_true"].astype(int)).to_numpy(dtype=int)
        z_ok = (k_pred_df["y_pred_zero"].astype(int) == k_pred_df["y_true"].astype(int)).to_numpy(dtype=int)

        s_freq = paired_diff_stats(m_ok, f_ok)
        s_pers = paired_diff_stats(m_ok, p_ok)
        s_zero = paired_diff_stats(m_ok, z_ok)
        print("  Baseline comparison (paired accuracy difference: model - baseline)")
        print(
            f"    vs freq-threshold: diff={s_freq['diff']:.3f}, "
            f"95%CI [{s_freq['ci_low']:.3f}, {s_freq['ci_high']:.3f}], "
            f"p={s_freq['p_value']:.4f}, discordant(model+,base-)={s_freq['n10']}, "
            f"(model-,base+)={s_freq['n01']}"
        )
        print(
            f"    vs persistence:   diff={s_pers['diff']:.3f}, "
            f"95%CI [{s_pers['ci_low']:.3f}, {s_pers['ci_high']:.3f}], "
            f"p={s_pers['p_value']:.4f}, discordant(model+,base-)={s_pers['n10']}, "
            f"(model-,base+)={s_pers['n01']}"
        )
        print(
            f"    vs always-zero:   diff={s_zero['diff']:.3f}, "
            f"95%CI [{s_zero['ci_low']:.3f}, {s_zero['ci_high']:.3f}], "
            f"p={s_zero['p_value']:.4f}, discordant(model+,base-)={s_zero['n10']}, "
            f"(model-,base+)={s_zero['n01']}"
        )
        print("  Strict predictions by year (K-emergence)")
        print(k_pred_df[["target_year", "y_true", "y_pred_model", "y_pred_freq", "y_pred_persist", "p_model", "k_freq"]].to_string(index=False))

    if RUN_NESTED_DIAGNOSTIC:
        print("\n[Optional] Nested CV Diagnostic (conservative estimate)")
        nested_df, nested_macro = nested_cv_diagnostic(train_df)
        if nested_df is None:
            print("  Not enough data for nested diagnostic.")
        else:
            print(f"  AUC:        {nested_macro['auc']:.3f}")
            print(f"  PR-AUC:     {nested_macro['pr_auc']:.3f}")
            print(f"  Precision:  {nested_macro['precision']:.3f}")
            print(f"  Recall:     {nested_macro['recall']:.3f}")
            print(f"  F1:         {nested_macro['f1']:.3f}")
            print(f"  Top-1 Hit:  {nested_macro['hit1']:.2f}")
            print(f"  Top-3 Hit:  {nested_macro['hit3']:.2f}")

    print("\nDone.")


